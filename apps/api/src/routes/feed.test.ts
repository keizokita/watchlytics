...
/**
 * Usuários próprios do teste, todos criados aqui. Os casos de A4 e A5 swipam o
 * catálogo inteiro — fazer isso numa conta compartilhada atropelaria os outros
 * arquivos de teste, que rodam em paralelo contra o mesmo banco.
 */
 ...
 * Numa conta compartilhada isso é corrida perdida: `swipes.test.ts` roda em
 * paralelo e apaga os swipes dela seis vezes, e como o score leva o peso por gênero
...
/** Gosto neutro: os casos que olham filtro, cursor e ruído não swipam nada. */
const NEUTRAL = "00000000-0000-4000-8000-0000000000a5";
const TEST_USERS = [
  LOVES_DRAMA,
  LOVES_ANIMATION,
  SAW_EVERYTHING,
...
test("A1: filtro inválido responde 400, não 500", async () => {
  const res = await app.inject({
    ...
    headers: { authorization: `Bearer ${signAccess(NEUTRAL)}` },
  });
...
test("A4: o ruído reordena entre uma abertura de feed e outra", async () => {
  const a = (await feed(NEUTRAL)).items.map((i) => i.id);
  const b = (await feed(NEUTRAL)).items.map((i) => i.id);
...
test("A5 degrau 1: relaxa ano → gênero → tipo e diz o que relaxou", async () => {
  ...
  const r = await feed(NEUTRAL, "?types=tv&genres=19&yearFrom=2024&languages=ko");
